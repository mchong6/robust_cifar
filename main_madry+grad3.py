...
if args.resume or args.test:
    ...
    checkpoint = torch.load('./checkpoint/max_feats.t7')
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        with torch.no_grad():
            real_feat, _ = net(inputs)
            
        eps = torch.zeros_like(inputs).normal_(-8/255, 8/255).cuda()
        for _ in range(10):
            eps.requires_grad = True
            feat, _ = net((inputs+eps).clamp(0,1))
            loss = F.mse_loss(feat[-1], real_feat[-1])
            grad = torch.autograd.grad(loss, [eps])[0]
            eps = eps + 1/255 * torch.sign(grad)
            eps = eps.clamp(-8/255, 8/255).detach()
            
        adv_im = (inputs+eps).clamp(0,1).detach()
        
        _, outputs = net(adv_im)
        loss = criterion(outputs, targets)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        grad_loss = torch.zeros_like(loss).cuda()
...
#         torch.save(state, './checkpoint/ckpt_grad_%.3f.t7'%args.lambda_grad)
        torch.save(state, './checkpoint/max_feats.t7')
        best_acc = acc
