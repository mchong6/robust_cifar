...
def calc_grad(inputs, targets, classifier):
    ...
    output = classifier(inputs) #[B, 10]
    sm_target = F.softmax(output, 1)[torch.arange(targets.size(0)).cuda(),targets]
    loss = criterion(output, targets)
        
    if args.grad:
#         gradients = torch.autograd.grad(loss, [inputs], create_graph=True)[0]
        gradients = torch.autograd.grad(outputs=sm_target, inputs=inputs,
                      grad_outputs=torch.ones(sm_target.size()).cuda(),
...
#    if args.grad:
##         gradients = torch.autograd.grad(loss, [inputs], create_graph=True)[0]
#        gradients = torch.autograd.grad(outputs=output, inputs=inputs,
...
#         if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/logits_%s_adv%d_grad%d_lambda_%.1f.t7'%(args.model, args.adv, args.grad, args.lambda_grad))
        best_acc = acc
